 ...
 * @param fields 配置项目
 */
const getAmTableField = (fields: Array<any>, ctrl?: AmTableField): Array<AmTableField> => {
...
export default forwardRef(function AmTable(props: AmTableProps, ref) {
  const {
    className,
    fields,
    header,
  ...
  /** 表格配置 */
  const amTableFields: Array<AmTableField> = getAmTableField(fields, ctrl)
  /** 表格数据 */
  ...
  return (
    <Card className={`am-table ${className}`}>
      {meta || btnBefore || children ? (
        <header className="am-table-header">
          <div className="am-table-header-left">
            <Space size="large">{meta && <h2 className="am-table-title">{meta.title}</h2>}</Space>
          </div>
          <div className="am-table-header-right">
            <Space>
              {btnBefore}
              {dataAnalysis
              ...
              {exportVisible && api && (
                <AmButton icon={<DownloadOutlined />} onClick={handleDownLoad}>
                  导出
                </AmButton>
              )}
              {children}
